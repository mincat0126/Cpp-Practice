...
> 만약 요청한 양만큼 메모리가 남아 있지 않을 경우는 에러를 리턴하여 응용 프로그램에게 메모리가 없다는 것을 알려준다.  
...
## 할당 및 해제
**malloc**: 인수로 필요한 메모리양을 바이트 단위로 전달하여 요청만큼 할당하는 함수    
&nbsp;&nbsp;&nbsp;**선언**: void *malloc(size_t size);  
> 사실상 이 함수로 할당할 수 있는 최대 용량은 4G다.   
> 사이즈 부분에는 상수 뿐 아니라 변수도 사용 가능하다.  
> 응용 프로그램이 필요한만큼 할당을 요청하면 사용되지 않은 빈 영역을 찾아 시작 번지를 리턴한다.  
> void *형을 리턴하며 받는 쪽에서는 원하는 타입으로 캐스팅해야 한다.  
...
# 재할당
**realloc**: 첫 번째 인수로 할당된 시작 번지를 받아 두 번째 인수로 재할당할 크기를 전달하는 함수.  
