...
> atoi 함수 인수의 전달되는 형태는 수치값이어야 한다.(0~9와 +- 가능)  
...
> strtol 함수의 두 번째 인수에 변환이 중지된 지점이 반환된다.
...
int main()
{
	...
	long i = strtol(str, &temp, 10);   //<실행결과>
	printf("%d\n", i);                 //1234
	printf("%s\n",temp);               //abc567
}</pre><br><br><br>
...
> 첫 번째 인수 buffer가 추가되어 있다는 점을 제외하고 사용하는 방법이나 %d, %f 등의 서식은 prinf 함수와 동일하다.
